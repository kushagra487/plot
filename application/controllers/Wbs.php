	...
	//
		...
		public function index() {
			...
			if($loggedin == TRUE) {
				...
				$data['pm_details'] = $this->wbs_model->get_assiged_pm_details($get_id);
				$user_id = $this->session->userdata['user_id'];
				$role = $this->session->userdata['role'];
				...
				if(isset($_POST['ucomment'])){
				    ...
				    redirect(base_url().'wbs/'. 'index/' . $this->uri->segment('3'));
				}				
				...
				//print_r($_POST);die;
				  if(isset($_POST['project_id'])){
				   ...
				   $mp_count=$counts['Mega Process'];
					...
					$counts = array_count_values($_POST['rowC2']);
					...
					$counter_mp=0;
					for($i=0;$i<count($_POST['rowC1']);$i++){
						
						if($_POST['rowC1'][$i]!=''){
							$counter_mp++;
							$sql_mp_ins="INSERT INTO mega_process (mp_name,project_id,last_modified,unique_code) 
					VALUES ('".addslashes($_POST['rowC1'][$i])."','".$this->uri->segment('3')."',now(),'".$counter_mp."')";
					...
					$values_mp=$this->wbs_list_model->findme($_POST['rowC1'],$k-1);
				...
				$sql_mp="select mp_id,unique_code from mega_process WHERE mp_name='".$parent_value_mp."' AND status=0 ORDER BY mp_id DESC LIMIT 1";
					...
					$new_array1=array();
					for($p1=0;$p1<count($_POST['rowC1']);$p1++){
					
						
						array_push($new_array1,$_POST['rowC1']);
					...
					$actid_unique++;
					$sql_actiity_ins="INSERT INTO activity (activity_name,planned_quantity,actually_quantity,start_date,finish_date,assigned_person,resources,template_reference,project_id,dependent_on,last_modified,template_document,comments) VALUES ('".addslashes($_POST['rowC'.$newj][$l])."','".$_POST['planned_quantity'][$l]."','".$_POST['actually_quantity'][$l]."','".$_POST['start_date'][$l]."','".$_POST['end_date'][$l]."','".$_POST['responsibilities'][$l]."','".$_POST['resources'][$l]."','".$_POST['template_reference'][$l]."','".$this->uri->segment('3')."','".$_POST['dependency'][$l]."',now(),'".$img_name."','".$_POST['comments'][$l]."')";
				...
				header("Location:".base_url().'wbs_list/'. 'index/' . $this->uri->segment('3'));
				...
				$sql_mp="select mp_id,unique_code from mega_process WHERE mp_name='".$parent_value_mp."' AND status=0 ORDER BY mp_id DESC LIMIT 1";
					...
					$sql1="select pid,unique_code from process WHERE process_name='".$value_my."'   AND project_id='".$this->uri->segment('3')."' AND status=0 and mp_id='".$resmp[0]->mp_id."'  ORDER BY pid DESC LIMIT 1";
					$query1=$this->db->query($sql1);
					$res1 = $query1->row(); 	
		...
		public function wbs_new(){
		$this->load->view('header', $data);

						$this->load->view('admin_sidebar', $data);

						$this->load->view('Wbs/wbs_new', $data);
