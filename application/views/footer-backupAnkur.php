...
function Forstart(savePapa){
	var saveStartDate = $(savePapa).parent().parent().find('.Enddatesel .magic').text();
			var arr = saveStartDate.split('/');
			var Dday = arr[0];
			var Mmonths = arr[1];
			var Yyear = arr[2];
...
$('.datesel').on('click',function(){
			var changeId = 'Startdatepickerhere';
			//alert(changeId);
           $('#mytimepicker').modal('show');
		   $('#mytimepicker').find('.modal-body>.date').attr('id',changeId);
...
var saveText= $(this).parents('#mytimepicker').find('input[type="text"]').val();
$(savePapa).find('.dateselect').attr('value',saveText);
$(savePapa).find('span.magic').empty();
//$(savePapa).after('<input type="hidden" name=""');
$(savePapa).find('input[type="hidden"]').attr('value',saveText);
$(savePapa).find('.dateselect').after('<span class="magic">'+saveText+'</span>');
...
function Forend(saveEPapa){
			...
			var Mmonth = arr[1];
			var Yyear = arr[2];
			var TotalDay = Mmonth+' '+Dday+' '+Yyear;
...
$('.Enddatesel').on('click',function(){
		...
		var changeId = 'endDatepickerhere';
		
		 $('#myEndtimepicker').find('.modal-body>.date').attr('id',changeId);

			saveEPapa = $(this);
			$(saveEPapa).find('span.magic').remove();
			$('#myEndtimepicker').modal('show');
			// $('.endDatepickerhere'+$ze).data('datetimepicker').Clear(null);
    		 Forend(saveEPapa);
...
var saveText= $(this).parents('#myEndtimepicker').find('input[type="text"]').val();
$(saveEPapa).find('.dateselect').attr('value',saveText);
$(saveEPapa).find('span.magic').empty();
$(saveEPapa).find('input[type="hidden"]').attr('value',saveText);
$(saveEPapa).find('.dateselect').after('<span class="magic">'+saveText+'</span>');
 $('#myEndtimepicker').modal('hide');
       ...
       /* $(".endDatepickerhere").on("dp.change", function (e) {
				 alert(e.date);
            $('.Startdatepickerhere').data("DateTimePicker").maxDate(e.date);
        });	*/
</script>
