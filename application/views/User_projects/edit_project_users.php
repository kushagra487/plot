			...
			<div class="x_panel">
				...
				<div class="x_content">
					<form class="form-horizontal form-label-left" method="post" novalidate enctype="multipart/form-data" action="">
						...
						<div class="item form-group">
							<div class="col-md-6 col-md-offset-2 col-md-6 col-sm-6 col-sm-offset-2 col-xs-12">
								<select class="select2_multiple form-control" name="pm_list[]" multiple="multiple">
									<?php
										foreach($pm_details as $key => $pm_details_value){
											//munesh
											//tanu
											if($pm_list){
												foreach($pm_list as $key => $pm_value){
													//tanu
													//monika
													?>
														<option value="<?php echo $pm_details_value['user_id'];?>" <?php if($pm_details_value['user_id'] == $pm_value['pm_list']){ echo "selected"; }?>><?php echo $pm_details_value['user_id'];?></option>
													<?php
												}
											}else{
												?>
													<option value="<?php echo $pm_details_value['user_id'];?>"><?php echo $pm_details_value['user_id'];?></option>
												<?php
											}
										}
									?>
                        ...
                        <div class="item form-group select-member">
							<div class="col-md-6 col-md-offset-2 col-md-6 col-sm-6 col-sm-offset-2 col-xs-12">
                         <textarea name="comment" role="" cols=""><?php echo $project_details['comment']; ?></textarea>
                         </div>
						</div>
