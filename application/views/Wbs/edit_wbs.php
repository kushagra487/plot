...
<div class="right_col" role="main">
  <h3 class="title text-uppercase">
    <?php echo $project_details['project_name']; ?> <img
      src="<?php echo base_url() ?>project_uploads/<?php echo $project_details['project_logo'] ?>"
    ...
    <form method="post" action="" class="date_filter_form">
        ...
        <div class="form-group">
          ...
          <select class="form-control customwidth" name="search_mp">
            ...
            <?php foreach ($all_mp as $all_mp) { ?>
              <option value="<?php echo $all_mp['mp_id'] ?>" <?php if ($_POST['search_mp'] == $all_mp['mp_id'])
                  echo "selected"; ?>>
                <?php echo $all_mp['mp_name'] ?>
              </option>
            <?php } ?>
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select class="form-control customwidth" name="search_process">
            ...
            <?php foreach ($all_process as $all_process) { ?>
              <option value="<?php echo $all_process['pid'] ?>" <?php if ($_POST['search_process'] == $all_process['pid'])
                  echo "selected"; ?>>
                <?php echo $all_process['process_name'] ?>
              </option>
            <?php } ?>
          </select>
        </div>
      ...
      <div class="col-md-2 col-sm-6 col-xs-12">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control customwidth" id="start_date_activity" name="start_date_activity"
            value="<?php echo $_POST['start_date_activity'] ? $_POST['start_date_activity'] : date("d/m/Y", strtotime($project_details['start_date'])); ?>"
      ...
      <div class="col-md-2 col-sm-6 col-xs-12">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control customwidth" id="end_date_activity" name="end_date_activity"
            value="<?php echo $_POST['end_date_activity'] ? $_POST['end_date_activity'] : date("d/m/Y", strtotime($project_details['end_date'])); ?>"
          ...
          <select name="responsilbe_person" class="form-control">
            ...
            <?php foreach ($responsible_person as $responsible_person) {
              if ($responsible_person['assigned_person'] != '') {
                ?>
                <option value="<?php echo $responsible_person['assigned_person'] ?>" <?php if ($_POST['responsilbe_person'] == $responsible_person['assigned_person'])
                    echo "selected"; ?>>
                  <?php echo $responsible_person['assigned_person'] ?>
                </option>
              <?php }
            } ?>
          </select>
  ...
  <div class="row">
      ...
      <div class="x_contents">
        <form name="wbs_form" id="wbsform" method="post" action="" enctype="multipart/form-data">
              ...
              <table width="100%" cellspacing="0" class="table2 table-bordered nowrap" id="datatable-keytable_wrapper">
                ...
                <thead>
                  <tr>
                    ...
                    <th class="cell13"><textarea rows="1" cols="10"
                        style="width:138px !important;">Finish Date & Time</textarea></th>
                    ...
                    <th><textarea name="col[]" cols="10" rows="1"> Dependency</textarea>
                      <div class="mtn_togg"><span class="toggle_insert"><i class="fa fa-angle-right"></i></span>
                        <span class="hover_toggle"><small class="insert_col">Insert</small><small
                            class="delete_col">Delete</small></span>
                      </div>
                    </th>
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <div class="mtn_togg icono"><span class="toggle_insert"><i class="fa fa-angle-right"></i></span>
                          <span class="hover_toggle"><small class="insert_row">Insert</small><small
                              class="delete_row">Delete</small></span>
                        </div>
                      </td>
                      ...
                      <td><input type='hidden' name='previous_document[]' value=''><input value=""
                          name="template_document[]" type="file">
                      </td>
                      <td>
          ...
          <input type="hidden" name="project_start_date" id="project_start_date"
            value="<?php echo date("d/m/Y", strtotime($project_details['start_date'])) ?>">
          <input type="hidden" name="project_end_date" id="project_end_date"
            value="<?php echo date("d/m/Y", strtotime($project_details['end_date'])) ?>">
          <?php if ($project_details['status'] == 0) { ?>
            <div class="container" style='margin-top:10px;'>
              <div class="form-group">
                <div class="form-check">
                  <input type="checkbox" class="form-check-input" name='reporting_manager_mail'
                    id='reporting_manager_mail'>
    ...
    margin-top: 10px;">Save</button>
            <button class="btn hidden" value="Submit" type="submit" name="btn_submit" id="confirmodif"
              style="background: #1d9f75!important;padding: 4px 8px;color: #fff;border: none;margin-top: 10px;">Submit</button>
